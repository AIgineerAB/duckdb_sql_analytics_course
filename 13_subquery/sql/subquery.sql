...
where grade_math > (select avg(grade_math) from staging.students) -- subquery in ()
order by grade_math
...
select *
from staging.students
where class_name IN  (select class_name from staging.teachers where teacher_name = 'Anna S.') -- subquery in ()
...
where s.grade_math > (select avg(grade_math) 
                      from staging.students
                      where class_name = s.class_name) -- subquery in ()
